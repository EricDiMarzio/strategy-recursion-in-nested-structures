...
// SOLUTION
// Since we are iterating over the array and returning a single value based on each element, this is an excellent use case for the reduce method. Taking advantage of the reduce method's special parameters - accumulator and current - we check if the current element is an array. If it is NOTm then we had the current value to the accumulator. If it IS an array, we recursively call sumeNestedArray on the element before adding it to our accumulator.
